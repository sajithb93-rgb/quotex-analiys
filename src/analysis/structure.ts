...
function calcPressure(data:Candle[]):{snap:PressureSnapshot;history:{index:number;buy:number;sell:number;delta:number}[]}{
 const start=Math.max(0,data.length-80);const history:{index:number;buy:number;sell:number;delta:number}[]=[];let cumulative=0;
...
export function seedCandles(count=180,start=Date.now()-count*60000,basePrice=1.084,volatility=0.0007):Candle[]{let price=basePrice;const out:Candle[]=[];for(let i=0;i<count;i++){const wave=Math.sin(i/17)*volatility*.28;const noise=(Math.random()-.48)*volatility;const drift=wave+noise;const open=price,close=Math.max(basePrice*.5,open+drift),wick=Math.max(volatility*.45,Math.abs(drift)*.65);const high=Math.max(open,close)+Math.random()*wick,low=Math.min(open,close)-Math.random()*wick;out.push({time:start+i*60000,open,high,low,close,volume:500+Math.random()*500});price=close}return out}
